...
class BasicSolver:
    ...
    @staticmethod
    def daterange_static(start_date, end_date):
        for n in range(int((end_date - start_date).days)):
            yield start_date + timedelta(n)
    ...
    def translateStr2Datetime(self, datetimeString, isFull):
        if datetimeString == "" or datetimeString == None:
            return False
        ...
        running_seconds = int(hour) * 3600 + int(minute) * 60 + int(second)
        total_seconds = today_seconds + running_seconds
        if isFull:
            return {
